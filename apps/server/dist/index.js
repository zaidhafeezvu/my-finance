...
var port = import_config.appConfig.port;
app.use(helmet());
app.use(import_cors.default({
  origin: import_config.appConfig.corsOrigin,
  credentials: true
}));
app.use(import_morgan.default("combined"));
...
app.get("/health", (req, res) => {
  res.json({
    ...
    environment: import_config.appConfig.nodeEnv
  });
});
app.get("/api", (req, res) => {
  res.json({
...
app.use((err, req, res, next) => {
  ...
  res.status(500).json({
    ...
    message: import_config.appConfig.nodeEnv === "development" ? err.message : "Internal server error"
  });
});
app.use("*", (req, res) => {
...
app.listen(port, () => {
  ...
  console.log(`\uD83D\uDCCA Environment: ${import_config.appConfig.nodeEnv}`);
  console.log(`\uD83C\uDF10 CORS Origin: ${import_config.appConfig.corsOrigin}`);
});
